...
#Structure of Plots
  ...
  #Coordinates - where dat is plotted
  #Themes - non-data related information (fonts, colors)


#Data

trump.approval <- read.csv("approval_topline.csv", stringsAsFactors = F)
...
#This is a unique instance because the x value is a date. Typically you would use the function coord_cartesian(x) for integers and doubles. For more information - ?coord_cartesian

#Theme


sc.trump.approval + geom_point(aes(modeldate, approve_estimate, color = "red")) + theme_minimal() + geom_smooth() + labs(y="Approval", x = "Date", title = "Donald Trump Presidential Approval") + theme_minimal()
